...
export interface AuroraTextEffectProps {
  ...
  /** Drop the blobs' overlay blend so small text takes the full colors. */
  vivid?: boolean;
  /** Override blob size/blur (e.g. "w-24 h-24 blur-sm") — the stock 60vw blobs
      only graze small text with their blur falloff, so scale them down to it. */
  blobClassName?: string;
  /** Clip the aurora colors to the letter shapes instead of blending layers.
      The stock effect needs an opaque backdrop (bg-white) to blend against,
      which shows as a plate on translucent surfaces; this mode has none.
      Uses gradientColors (raw CSS colors), not the Tailwind bg-* classes. */
  clipText?: boolean;
    ...
    return (
      <div className={cn("flex items-center justify-center overflow-hidden", className)}>
        ...
        <h2
          ...
          style={{
            fontSize,
            backgroundImage: `linear-gradient(90deg, ${gradientColors.join(", ")}, ${gradientColors[0]})`,
            backgroundSize: "300% 100%",
            WebkitBackgroundClip: "text",
            backgroundClip: "text",
            color: "transparent",
            animation: `aurora-pan ${(animationSpeed.first ?? 5) * 2}s ease-in-out infinite`,
          }}
        >
          {text}
        </h2>
